...
export default function Login() {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex items-center justify-center p-4 sm:p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10 mobile-header-safe">
        <div className="text-center mb-8 sm:mb-10">
          <div className="inline-flex items-center gap-2 sm:gap-3 mb-4 sm:mb-6">
            <Heart className="w-8 h-8 sm:w-10 sm:h-10 text-white" />
            <span className="text-2xl sm:text-3xl font-black tracking-tighter">PULSE<span className="text-cyan-400">AI</span></span>
          </div>
          <h1 className="text-3xl sm:text-4xl font-black">{isRegister ? 'JOIN US' : 'WELCOME'}</h1>
          <p className="text-cyan-300 text-xs tracking-[0.3em] mt-2">YOUR HEALTH. YOUR RULES.</p>
        </div>

        <div className="bg-gray-900 rounded-2xl p-5 sm:p-8 border border-white/10">
          <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
            {isRegister && (
              <div className="relative">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-cyan-400" />
                <input type="text" placeholder="FULL NAME" value={name} onChange={e => setName(e.target.value)}
                  className="w-full pl-12 pr-4 py-4 bg-black border border-white/10 rounded-xl text-white placeholder-gray-600 focus:border-cyan-500 text-base tracking-wide" required />
              </div>
            )}
            <div className="relative">
            ...
            <div className="relative">
              ...
              <input type={showPass ? 'text' : 'password'} placeholder="PASSWORD" value={password} onChange={e => setPassword(e.target.value)}
                className="w-full pl-12 pr-14 py-4 bg-black border border-white/10 rounded-xl text-white placeholder-gray-600 focus:border-cyan-500 text-base tracking-wide" required />
              <button type="button" onClick={() => setShowPass(!showPass)} className="absolute right-4 top-1/2 -translate-y-1/2 text-cyan-400 hover:text-white p-1 min-w-[44px] min-h-[44px] flex items-center justify-center -mr-2">
                {showPass ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
              </button>
            </div>
            {error && <p className="text-red-400 text-sm text-center bg-red-500/20 p-2 rounded-lg">{error}</p>}
            <button type="submit" disabled={loading}
              className="w-full py-4 bg-white text-black font-black text-sm tracking-wider rounded-xl hover:bg-cyan-400 active:bg-cyan-500 transition-all flex items-center justify-center gap-2 disabled:opacity-50 min-h-[52px]">
          ...
          <p className="mt-5 sm:mt-6 text-center text-cyan-300 text-sm">
            {isRegister ? 'Have an account?' : 'No account?'}
            <button onClick={() => setIsRegister(!isRegister)} className="text-cyan-400 font-bold ml-2 p-1">{isRegister ? 'LOGIN' : 'REGISTER'}</button>
          </p>
        </div>
      </div>
    </div>
