...
import { 
  ...
  FileSearch, IndianRupee, Check, Crown, Star
} from 'lucide-react';
...
export default function Home() {
  ...
  const features = [
    ...
    { icon: Crown, title: 'FOR QUEENS', desc: 'Womens Health', path: '/queens', gradient: 'from-pink-500 to-purple-600', glow: 'pink', explanation: 'Dedicated healthcare features for women in India - from PCOS/PCOD management to prenatal maternal care and anemia detection.' },
  ...
  const upcomingItems = [
    ...
    { title: 'Dr. Smith Checkup', time: '2:30 PM', icon: Calendar, color: 'text-green-400', bg: 'bg-green-500/10' },
  ...
  return (
    <div className="min-h-screen bg-black text-white relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none">
        ...
        <div className="absolute w-[600px] h-[600px] -top-40 -right-40 bg-cyan-500/10 rounded-full blur-3xl animate-pulse" />
        <div className="absolute w-[600px] h-[600px] -bottom-40 -left-40 bg-purple-500/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '1s' }} />
      ...
      <header className="relative z-20 px-6 py-4 flex items-center justify-between border-b border-white/10 bg-black/50 backdrop-blur-xl">
        <Link to="/" className="flex items-center gap-3">
          ...
          <span className="text-2xl font-black tracking-tighter">PULSE<span className="text-cyan-400">AI</span></span>
        </Link>
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            {showNotifications && (
              <div className="absolute right-0 mt-2 w-80 bg-gray-900/95 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-2xl z-50 animate-fade-in">
          ...
          <div className="relative">
            <button 
              ...
              className="flex items-center gap-3 p-2 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 transition-colors"
            >
              <div className="w-9 h-9 bg-gradient-to-br from-cyan-500 to-blue-500 rounded-lg flex items-center justify-center">
                <User className="w-5 h-5 text-white" />
              </div>
              <span className="hidden md:block font-bold text-sm">{user?.full_name?.split(' ')[0] || 'User'}</span>
              <Menu className="w-4 h-4 text-gray-400" />
            </button>

            {showMenu && (
              <div className="absolute right-0 mt-2 w-56 bg-gray-900/95 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden shadow-2xl z-50">
                ...
                <div className="p-2">
                  ...
                  <button 
                    onClick={logout}
                    className="w-full flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 transition-colors text-left text-red-400"
                  >
      ...
      <main className="relative z-10 px-6 py-8 max-w-7xl mx-auto pb-24 md:pb-8">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
            <div className="animate-slide-in-left">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-xs font-bold tracking-[0.2em] text-cyan-400 uppercase">{greeting()}</span>
              </div>
            ...
            <div className="text-left md:text-right animate-slide-in-left" style={{ animationDelay: '0.1s' }}>
              <p className="text-3xl font-bold tabular-nums">{currentTime.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
              <p className="text-gray-500 text-sm">{currentTime.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
            </div>
          </div>
        </section>
        ...
        <section className="mb-10">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {quickStats.map((stat, index) => (
              <div 
                key={index}
                className="group relative p-5 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 hover:border-cyan-500/30 transition-all duration-500 animate-bounce-in hover-lift overflow-hidden"
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <div className="text-2xl font-black group-hover:text-cyan-400 transition-colors">{stat.value}<span className="text-sm text-gray-500 ml-1">{stat.unit}</span></div>
        ...
        <section className="mb-10 relative z-30">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-xs font-bold tracking-[0.3em] text-gray-500 uppercase">Quick Access</h2>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            {features.map((feature, index) => (
              ...
              >
                ...
                {flippedCard !== index ? (
                  /* Front of Card */
                  <div className="absolute inset-0 p-6 bg-white/5 border border-white/10 rounded-2xl hover:border-cyan-500/30 transition-all duration-300">
                    <div className="h-full flex flex-col">
                      <Link to={feature.path} className="flex-grow">
                        ...
                        <h3 className="font-black text-sm tracking-wide mb-1 group-hover:text-cyan-400 transition-colors duration-300">{feature.title}</h3>
                  ...
                  /* Back of Card */
                  <div className="absolute inset-0 p-6 bg-gradient-to-br from-gray-900 to-black border border-cyan-500/30 rounded-2xl">
                    <div className="h-full flex flex-col">
                      ...
                      <h4 className="font-black text-sm text-cyan-400 mb-2">{feature.title}</h4>
                      <p className="text-xs text-gray-300 leading-relaxed flex-grow">{feature.explanation}</p>
        ...
        <section className="mb-10">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-xs font-bold tracking-[0.3em] text-gray-500 uppercase">Today's Schedule</h2>
            <Link to="/medicine" className="text-cyan-400 text-xs font-bold hover:underline flex items-center gap-1 group">
              View All
          ...
          <div className="space-y-3">
            {upcomingItems.map((item, index) => (
              <div 
                key={index}
                className="group flex items-center gap-4 p-4 bg-white/5 border border-white/10 rounded-xl hover:bg-white/10 hover:border-cyan-500/20 transition-all duration-500 animate-slide-in-left hover-lift"
        ...
        <section className="grid md:grid-cols-2 gap-4">
          <Link 
            to="/emergency"
            className="relative p-6 bg-gradient-to-br from-red-500/10 to-orange-500/10 border border-red-500/20 rounded-2xl overflow-hidden group hover:scale-[1.02] transition-transform duration-300 animate-bounce-in"
          ...
          <Link 
            to="/consultation"
            className="relative p-6 bg-gradient-to-br from-cyan-500/10 to-blue-500/10 border border-cyan-500/20 rounded-2xl overflow-hidden group hover:scale-[1.02] transition-transform duration-300 animate-bounce-in"
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="font-bold text-lg">Start Consultation</h3>
                <p className="text-gray-400 text-sm">Get instant AI advice</p>
              </div>
            </div>
          </Link>
        </section>
        ...
        <section className="mt-12 mb-10">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400 text-sm">Affordable healthcare for everyone</p>
          </div>

          <div className="grid md:grid-cols-3 gap-6">
            {/* Free Plan */}
            <div className="relative p-6 bg-white/5 border border-white/10 rounded-2xl hover:border-cyan-500/30 transition-all duration-300 group">
              ...
              <button className="w-full py-3 bg-white/10 text-white rounded-xl font-semibold hover:bg-white/20 transition-all">
                Current Plan
              </button>
            </div>

            {/* Pro Plan - Popular */}
            <div className="relative p-6 bg-gradient-to-br from-cyan-500/10 to-purple-500/10 border-2 border-cyan-500/50 rounded-2xl transform md:-translate-y-2 shadow-xl shadow-cyan-500/10">
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 bg-gradient-to-r from-cyan-500 to-purple-500 rounded-full text-xs font-bold flex items-center gap-1">
            ...
            {/* Family Plan */}
            <div className="relative p-6 bg-white/5 border border-white/10 rounded-2xl hover:border-purple-500/30 transition-all duration-300 group">
              <div className="flex items-center gap-3 mb-4">
          ...
          <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="p-4 bg-white/5 rounded-xl text-center">
              ...
              <p className="font-bold text-green-400">90% on Meds</p>
            </div>
            <div className="p-4 bg-white/5 rounded-xl text-center">
              <Shield className="w-6 h-6 text-cyan-400 mx-auto mb-2" />
              <p className="text-xs text-gray-400">Data</p>
              <p className="font-bold text-cyan-400">100% Secure</p>
            </div>
            <div className="p-4 bg-white/5 rounded-xl text-center">
              <Clock className="w-6 h-6 text-purple-400 mx-auto mb-2" />
              <p className="text-xs text-gray-400">Available</p>
              <p className="font-bold text-purple-400">24/7</p>
            </div>
            <div className="p-4 bg-white/5 rounded-xl text-center">
              <Activity className="w-6 h-6 text-pink-400 mx-auto mb-2" />
              <p className="text-xs text-gray-400">Response</p>
              <p className="font-bold text-pink-400">&lt; 2 Seconds</p>
            </div>
          </div>
        </section>
      </main>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 bg-black/90 backdrop-blur-xl border-t border-white/10 px-4 py-3 md:hidden">
        <div className="flex items-center justify-around">
          <Link to="/dashboard" className="flex flex-col items-center gap-1 text-cyan-400 relative">
            <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-8 h-1 bg-cyan-400 rounded-full" />
            <HomeIcon className="w-5 h-5" />
            <span className="text-[10px] font-bold">Home</span>
          </Link>
          <Link to="/consultation" className="flex flex-col items-center gap-1 text-gray-500 hover:text-cyan-400 transition-all duration-300 hover:scale-110">
            <Brain className="w-5 h-5" />
            <span className="text-[10px]">AI Doc</span>
          </Link>
          <Link to="/medicine" className="flex flex-col items-center gap-1 text-gray-500 hover:text-cyan-400 transition-all duration-300 hover:scale-110">
            <Pill className="w-5 h-5" />
            <span className="text-[10px]">Meds</span>
          </Link>
          <Link to="/mental-health" className="flex flex-col items-center gap-1 text-gray-500 hover:text-cyan-400 transition-all duration-300 hover:scale-110">
            <Heart className="w-5 h-5" />
            <span className="text-[10px]">Mental</span>
          </Link>
          <Link to="/emergency" className="flex flex-col items-center gap-1 text-gray-500 hover:text-red-400 transition-all duration-300 hover:scale-110">
